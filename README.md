...
一个基于 **Android WebView** 的 Discuz! X3.5 小说论坛壳应用，当前版本 **v1.3.19**。

## 主要功能

- **版面列表返回不丢页（结构化方案）**：版块列表底部由站点自动加载出来的后续页，在「进帖子 → 返回」后原样还在。做法是：点帖子时帖子在新的独立界面里打开，版面列表所在的界面只被覆盖、不被销毁，返回时它的多页内容与滚动位置原封不动——不依赖任何还原脚本，因此也不会有「只加载几页」或卡顿的问题。（Android WebView 不支持 BFCache，返回一定会重建文档，所以任何 JS 快照/重滚还原都不可靠）
